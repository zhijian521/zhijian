...
- 适用范围：当前项目
- 最后核对：2026-07-18
- 维护范围：单篇和全部文章 ZIP 导出及图片归档规则
- 关联代码：`src/app/api/admin/posts/export/route.ts`、`src/components/modules/admin/post-management-client.tsx`
...
- 图片路径先经 `resolveUploadFilePath()` 校验：解析结果必须在 `public/uploads/` 内，`..` 等穿越路径直接拒绝，记入 `missingFiles`
...
| `src/lib/domain/uploads.ts`                               | `resolveUploadFilePath()` 防路径穿越校验       |
