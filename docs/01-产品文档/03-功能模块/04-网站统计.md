...
- 适用范围：当前项目
- 最后核对：2026-07-19
- 维护范围：统计采集、聚合、概览、访问记录和站点管理
- 关联代码：`public/script.js`、`src/app/admin/analytics/`、`src/app/api/admin/analytics/`、`src/lib/domain/analytics-aggregate.ts`、`src/lib/domain/analytics-query.ts`

知简后台的网站统计模块由统计概览页和站点管理页组成。两个路由页均由 Server Component 渲染统一页面头部并读取站点首屏数据；Client Component 负责筛选、图表、表格、CRUD 和接入代码交互。
...
**文件**：`src/lib/domain/analytics-aggregate.ts`（`aggregateDaily` / `ensureAggregated`）
...
路由页服务端读取站点选项并只向客户端传递 `id`、`name`；Dashboard 使用首个站点作为默认选项，再按所选时间范围请求概览或访问记录。
...
路由页服务端加载
  │
  ▼
listTrackSites() → initialSites（仅 id、name）
  │
  ▼
客户端挂载后 fetchData() — GET /api/admin/analytics/overview?siteId=xxx&range=7d
...
切换到访问记录 Tab
  │
  ▼
fetchVisits() — GET /api/admin/analytics/visits?siteId=xxx&range=7d&page=1&pageSize=10
```
...
路由页服务端读取站点列表作为首屏数据，客户端不重复请求首屏，后续创建、编辑、启停和删除后再刷新列表。
...
| 方法     | 路径                              | 说明                       |
| -------- | --------------------------------- | -------------------------- |
| `GET`    | `/api/admin/analytics/sites`      | 站点列表（排除 deleted）   |
| `POST`   | `/api/admin/analytics/sites`      | 创建站点（自动生成 ID）    |
| `PUT`    | `/api/admin/analytics/sites/{id}` | 更新站点（名称/域名/状态） |
| `DELETE` | `/api/admin/analytics/sites/{id}` | 软删除站点                 |
...
`GET /api/admin/analytics/overview` 聚合后 `Promise.all` 并行执行 12 个查询，`skipAggregate=true` 避免重复聚合：
...
| `src/app/api/admin/analytics/sites/route.ts`                  | 站点列表/创建 API                        |
| `src/app/api/admin/analytics/sites/[id]/route.ts`             | 站点更新/删除 API（路径参数）            |
| `src/lib/domain/analytics-aggregate.ts`、`analytics-query.ts`  | 日聚合写入 + 查询读取（经 analytics.ts 门面导出） |
