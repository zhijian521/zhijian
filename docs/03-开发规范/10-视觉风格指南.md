...
- 适用范围：当前项目
- 最后核对：2026-07-18
- 维护范围：全站视觉语言、色彩、排版和组件规格
- 关联代码：`src/app/tokens.css`、`src/app/theme.css`、`src/components/`
...
| 导航页分屏           | 四屏各 `100vh`，左侧纯文字导航展示当前屏并提供点击/键盘跳转；不展示索引标题和序号            |
| 导航页首屏           | 仅保留主题色品牌名、搜索与书签，内容从视口上方约 `22vh` 开始，不放说明性眉题和副文案         |
| 导航页首屏宽度       | 搜索栏 `max-width: 42rem`，书签区 `max-width: 54rem`，通过宽度差形成错落层次                 |
| 导航页工作区         | AI、备忘录和笔记直接占满可用高度，不增加页面级页头；桌面端左右内边距统一为 `6rem`            |
| 导航页输入聚焦       | 搜索、登录/注册和 AI 对话复用 `TextInput` 的朱砂边框与 `var(--ring)`；待办和笔记暂不接入     |
| 后台背景             | `.layout` 固定铺 `bg-nav.webp` 底图 + `--overlay-gradient` 蒙版（复用登录页/导航页范式）    |
| 后台侧边栏           | `width: 16rem (256px)`, `fixed`, `border-right: 1px solid var(--border)`, `bg: transparent`（直接透出背景纱罩） |
| 后台主内容区         | `margin-left: 16rem`, `padding: 3rem`, `min-height: 100vh`, `bg: transparent`（直接透出背景纱罩，内部卡片仍为实心，保证密集内容可读） |
...
_最后更新: 2026-06-27（补充面包屑、相关文章网格布局；统一 itemCategory/relatedCardMeta 字号为 0.75rem）_
